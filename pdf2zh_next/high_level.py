...
from pdf2zh_next.windows_no_console import enable_no_console_subprocesses
...
def _prefer_no_console_subprocess_executable() -> None:
    enable_no_console_subprocesses()
...
def _ensure_subprocess_pythonpath() -> None:
    ...
    if not site_packages.exists():
        return
    entries = [str(site_packages), str(runtime_root)]
...
def _send_subprocess_status(
    ...
    try:
        ...
        logger.debug("Failed to send subprocess status", exc_info=True)
...
def _translate_wrapper(
    ...
    try:
        _prefer_no_console_subprocess_executable()
        ...
        _send_subprocess_status(pipe_progress_send, "初始化 BabelDOC 翻译子进程")
        config = create_babeldoc_config(settings, file)
        _send_subprocess_status(
            pipe_progress_send,
            "正在加载 BabelDOC 文档解析模型",
            stage_current=1,
            stage_total=2,
...
async def _translate_in_subprocess(
    ...
    logger.info("Started translation subprocess pid=%s", translate_process.pid)
    try:
        pipe_progress_send.close()
    except Exception as e:
        logger.debug(f"Failed to close parent progress send pipe: {e}")
    try:
        pipe_cancel_message_recv.close()
    except Exception as e:
        logger.debug(f"Failed to close parent cancel receive pipe: {e}")
    cancel_flag = False
    try:
