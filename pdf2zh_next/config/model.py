    ...
    def validate_settings(self) -> None:
        ...
        for file in self.basic.input_files:
            ...
            if file_path.suffix.lower() not in allowed_extensions:
                raise ValueError(f"File must be a PDF or Markdown file: {file}")
        ...
        if self.mineru.enabled:
            mineru_backend = self.mineru.backend
            allowed_mineru_backends = {
                "transformers",
                "http-client",
                "online-api",
                "online-agent",
            }
            if mineru_backend not in allowed_mineru_backends:
                raise ValueError(
                    f"Invalid MinerU backend: {mineru_backend}. "
                    f"Expected one of {sorted(allowed_mineru_backends)}"
                )
            ...
            if mineru_backend == "online-api" and not (
                ...
                raise ValueError(
                    "MinerU online-api backend requires api_token or MINERU_API_TOKEN"
                )
